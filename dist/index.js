...
let run = (() => {
  var _ref = (0, _bluebird.coroutine)(function* () {
    ...
    const releases = yield fetcher.fetchChangelog();

    formatChangelog(releases).forEach(function (line) {
...
program.option('-b, --base-branch <name>', '[optional] specify the base branch name - master by default').option('-f, --future-release <version>', '[optional] specify the next release version').option('-t, --future-release-tag <name>', '[optional] specify the next release tag name if it is different from the release version').option('-l, --labels <names>', '[optional] labels to filter pull requests by', val => val.split(',')).option('-o, --owner <name>', '[optional] owner of the repository').option('-r, --repo <name>', '[optional] name of the repository').description('Run GitHub changelog generator.').parse(process.argv);
